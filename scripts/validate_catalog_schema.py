...
def _assert_type(value, expected, path: str, errors: list[str]) -> None:
...
def validate_catalog(path: Path = CATALOG_PATH) -> list[str]:
    ...
    for cat_key, cat_data in data["categories"].items():
        ...
        for idx, cmd in enumerate(commands):
            ...
            if isinstance(cmd.get("prerequisites"), list) and any(not isinstance(t, str) for t in cmd["prerequisites"]):
