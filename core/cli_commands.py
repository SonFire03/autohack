...
def cli_check() -> None:
    catalog, _, _ = _get_core()
    timeout_s = __import__("core.config_manager", fromlist=["ConfigManager"]).ConfigManager().get("command_timeout")
...
def cli_tag(tag: str) -> None:
    ...
    results = [cmd for cmd in catalog.get_all() if tag_norm in [t.lower() for t in cmd.get("tags", [])]]
    if not results:
